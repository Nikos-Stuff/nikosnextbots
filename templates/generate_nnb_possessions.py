...
for index, item in enumerate(original_list):
    ...
    print(f"execute at @e[type=minecraft:zombie,tag={item}_z] if entity @e[type=minecraft:snowball,distance=..2.2] run execute at @a[distance=..5.5,sort=nearest,tag=!nextbot,limit=1] run scoreboard players set @p nn_possesion {index + 1}")
