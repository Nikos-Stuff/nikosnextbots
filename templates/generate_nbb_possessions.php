original_list = [
    ...
    "winrar", "fan", "boykisser", "subspace_tripmine", "nuke", "nothing", "flash", "osc",
...
for index, item in enumerate(original_list):
    ...
    print(f"execute at @e[type=minecraft:zombie,tag=nextbot] if entity @e[type=minecraft:snowball,distance=..2.2] run execute at @a[distance=..5.5,sort=nearest,tag=!nextbot] run scoreboard players set @p nn_possesion {index + 1}")
    print(f"execute at @e[type=minecraft:zombie,tag=nextbot] if entity @e[type=minecraft:snowball,distance=..2.2] run tag @e[type=zombie,limit=1,distance=..1.1] add controled")
    print(f"execute at @e[type=minecraft:zombie,tag=nextbot] if entity @e[type=minecraft:snowball,distance=..2.2] run kill @e[tag=controled,distance=..1.1,limit=1]")
